...
class DualVideoStarter:
    ...
    def __init__(self, stream_address: str, stream_port: str, mux: Mux, pose_track: PoseDetectionTrack, file: str, connection_offers: dict, connection_answers: dict):
    ...
    async def start(self, video_name: str, score_aggregator: ScoreAggregator, user_id: str):
        ...
        self.pose_track.set_feedback(keypoint_feedback)

        self.streamer = await self.mux.create_dual_stream(offer, keypoint_feedback, self.pose_track, recorder, self.connection_offers, self.connection_answers, filepath, **kwargs)
